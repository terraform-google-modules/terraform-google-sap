...
resource "google_compute_address" "sapdapp11-1" {
  ...
  name         = "${length(var.app_vm_names) > (0 + (count.index * 2)) ? var.app_vm_names[0 + (count.index * 2)] : "${var.vm_prefix}app1${1 + (count.index * 2)}"}-internal"
...
resource "google_compute_address" "sapdapp12-1" {
  address_type = "INTERNAL"
  count        = var.app_vms_multiplier
  name         = "${length(var.app_vm_names) > (1 + (count.index * 2)) ? var.app_vm_names[1 + (count.index * 2)] : "${var.vm_prefix}app1${2 + (count.index * 2)}"}-internal"
  project      = data.google_project.sap-project.project_id
  region       = var.region_name
  subnetwork   = data.google_compute_subnetwork.sap-subnet-app-1.self_link
}
...
resource "google_compute_disk" "sapdapp11_export_interfaces" {
  ...
  project          = data.google_project.sap-project.project_id
  provisioned_iops = var.app_disk_type == "hyperdisk-extreme" ? max(10000, 2 * (lookup(var.disk_size_map, "app_disk_export_interfaces_size", var.app_disk_export_interfaces_size))) : null
  size             = lookup(var.disk_size_map, "app_disk_export_interfaces_size", var.app_disk_export_interfaces_size)
  timeouts {
    create = "1h"
    delete = "1h"
    update = "1h"
...
resource "google_compute_disk" "sapdapp11_usr_sap" {
  ...
  project          = data.google_project.sap-project.project_id
  provisioned_iops = var.app_disk_type == "hyperdisk-extreme" ? max(10000, 2 * (lookup(var.disk_size_map, "app_disk_usr_sap_size", var.app_disk_usr_sap_size))) : null
  size             = lookup(var.disk_size_map, "app_disk_usr_sap_size", var.app_disk_usr_sap_size)
  timeouts {
    create = "1h"
    delete = "1h"
    update = "1h"
...
resource "google_compute_disk" "sapdapp12" {
  ...
  name    = length(var.app_vm_names) > (1 + (count.index * 2)) ? var.app_vm_names[1 + (count.index * 2)] : "${var.vm_prefix}app1${2 + (count.index * 2)}"
  project = data.google_project.sap-project.project_id
  size    = length(regexall("metal|c4-", var.app_machine_type)) > 0 ? 64 : 50
  timeouts {
    create = "1h"
