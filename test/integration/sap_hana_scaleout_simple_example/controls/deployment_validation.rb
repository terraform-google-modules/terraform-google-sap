...
# See the License for the specific language governing permissions and
# limitations under the License.
control 'deployment_validation_master' do

  describe command("gcloud compute instances get-serial-port-output #{attribute('instance_master_name')} --project=#{attribute('project_id')} --zone=#{attribute('zone')}") do
      ...
      context "output of sapcontrol -nr 10 -function GetSystemInstanceList command" do
        its('stdout') { should match('HDB|HDB_WORKER, GREEN') }
      end
    end
end
