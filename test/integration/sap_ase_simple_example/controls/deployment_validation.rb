...
control 'deployment_validation' do
    ...
    describe command("gcloud compute instances get-serial-port-output #{attribute('instance_name')} --project=#{attribute('project_id')} --zone=#{attribute('zone')}") do
      ...
      context "output of df -h command" do
        its('stdout') { should match('meta-data=/dev/vg_asesid/vol') }
        its('stdout') { should match('meta-data=/dev/vg_asesapdata/vol') }
        its('stdout') { should match('meta-data=/dev/vg_aselog/vol') }
        its('stdout') { should match('meta-data=/dev/vg_asesaptemp/vol') }
        its('stdout') { should match('meta-data=/dev/vg_asesapdiag/vol') }
        its('stdout') { should match('meta-data=/dev/vg_asebackup/vol') }
